...
typedef MeshLib::CBaseMesh<MeshLib::CVertexUVRGB, MeshLib::CEdge, MeshLib::CFace, MeshLib::CHalfEdge> M;
typedef MeshLib::CIterators<M> It;

MeshLib::CMeshViewer * pViewer;
AC::VecStr filesObj;
AC::VecStr filesTexture;
M * pMesh = NULL;
...
void changeToNextFrame() {
	...
	if (hasTexture)
	{
		pViewer->setTexture(filesTexture[frameId].c_str());
	}
	if (showFrameId)
	{
...
int main(int argc, char ** argv) {
	if (argc < 3) {
		printf("Need 2 input file: [path to mesh] [path to texture].\n");
		return -1;
	}
	...
	AC::IO::getFilesWithExt(argv[2], "bmp", filesTexture);
	std::sort(filesObj.begin(), filesObj.end());
	std::sort(filesTexture.begin(), filesTexture.end());
	hasTexture = false;
	hasTexture = true;
	...
	pViewer = &viewer;
	viewer.setting().vertexColorMode = MeshLib::GLSetting::userDefined;
	viewer.setting().vertexSize = 5;
	viewer.setMeshPointer(&m, true, false, true);
	if (hasTexture)
	{
		viewer.setTexture(filesTexture.front().c_str());
	}
